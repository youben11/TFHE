...
@pytest.mark.parametrize("sigma", [2 ** -15, 2 ** -30])
def test_trlwe_sub_encrypted(u1, u2, n, sigma):
    u1 = TorusPolynomial.from_real(u1)
    u2 = TorusPolynomial.from_real(u2)
    expected = u1 - u2
    sk = RLWESecretKey(big_n, k)
    p = 2 ** 16
    c1 = TLWE(n, sigma, p)
...
@pytest.mark.parametrize("n", [600, 800, 1024, 2048])
@pytest.mark.parametrize("sigma", [2 ** -15, 2 ** -30])
def test_trlwe_sub_plain(u1, u2, n, sigma):
    u1 = TorusPolynomial.from_real(u1)
    u2 = TorusPolynomial.from_real(u2)
    expected = u1 - u2
    sk = RLWESecretKey(big_n, k)
...
@pytest.mark.parametrize("n", [600, 800, 1024, 2048])
@pytest.mark.parametrize("sigma", [2 ** -15, 2 ** -30])
def test_trlwe_mul_int(u1, i2, n, sigma):
    p = 2 ** 16
